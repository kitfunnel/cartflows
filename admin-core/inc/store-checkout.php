...
class StoreCheckout {
	...
	public function __construct() {
		...
		add_filter( 'cartflows_admin_exclude_import_meta_keys', array( $this, 'exclude_import_meta_keys' ), 10, 2 );
	...
	/**
	 * Updating exclude meta array for re import templates
	 *
	 * @param array $meta_array existing array of exclude meta.
	 * @param int   $step_id current step id.
	 * @return array
	 * @since X.X.X
	 */
	public function exclude_import_meta_keys( $meta_array, $step_id ) {
		if ( 'yes' === get_post_meta( $step_id, 'cartflows_imported_step', true ) ) {
			return array_merge(
				$meta_array,
				array(
					'wcf_fields_billing',
