...
class Cartflows_Checkout_Markup {
	...
	public function is_modern_checkout_layout( $checkout_id = '' ) {
		...
		if ( ! empty( $post ) && empty( $checkout_id ) ) {
			$checkout_id = $post->ID;
		} elseif ( wp_doing_ajax() && isset( $_POST['id'] ) ) { //phpcs:ignore

			$is_checkout = wcf()->utils->check_is_checkout_page( intval( wp_unslash( $_POST['id'] ) ) ); //phpcs:ignore

			if ( $is_checkout ) {
				$checkout_id     = intval( wp_unslash( $_POST['id'] ) ); //phpcs:ignore
				$checkout_layout = isset( $_POST['layout'] ) ? sanitize_text_field( wp_unslash( $_POST['layout'] ) ) : ''; //phpcs:ignore
			}
		}
